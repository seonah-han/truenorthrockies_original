...
<form id="yith-wcwl-form" action="<?php echo $form_action ?>" method="post" class="woocommerce">
	...
	if( ! empty( $page_title ) ) :
		...
		<div class="wishlist-title <?php echo ( $is_custom_list ) ? 'wishlist-title-with-form' : ''?>">
			<?php echo apply_filters( 'yith_wcwl_wishlist_title', '<h5>' . $page_title . '</h5>' ); ?>
				...
				foreach( $wishlist_items as $item ) :
					...
					if( $product && $product->exists() ) :
						...
						<tr id="yith-wcwl-row-<?php echo $item['prod_id'] ?>" data-row-id="<?php echo $item['prod_id'] ?>">
							...
							<td class="product-name" data-title="<?php echo apply_filters( 'yith_wcwl_wishlist_view_name_heading', __( 'Product Name', 'yith-woocommerce-wishlist' ) ) ?>">
							...
							<?php if( $show_price ) : ?>
								<td class="product-price" data-title="<?php echo apply_filters( 'yith_wcwl_wishlist_view_price_heading', __( 'Unit Price', 'yith-woocommerce-wishlist' ) ) ?>">
									<?php echo $product->get_price() ? $product->get_price_html() : apply_filters( 'yith_free_text', __( 'Free!', 'yith-woocommerce-wishlist' ) ); ?>
								</td>
							<?php endif ?>

							<?php if( $show_stock_status ) : ?>
								<td class="product-stock-status" data-title="<?php echo apply_filters( 'yith_wcwl_wishlist_view_stock_heading', __( 'Stock Status', 'yith-woocommerce-wishlist' ) ) ?>">
...
<?php if ( $show_cb || $is_user_owner && $show_ask_estimate_button && $count > 0  ): ?>
	<tfoot>
		<tr>
